...
.DEFAULT_GOAL := help
.PHONY: help test lint fmt fmt-check clippy wasm build-web check corpora bench report clean
...
bench: ## [E5] Benchmarks criterion de encode/decode por b
	@echo "make bench: ainda não existe — é a Etapa E5 de docs/03-ETAPAS.md." >&2
	@exit 2
